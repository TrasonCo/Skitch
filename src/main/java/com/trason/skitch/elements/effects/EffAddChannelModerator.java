...
@Examples("add twitch channel moderator \"username\"")
@Since("1.4.1")
public class EffAddChannelModerator extends Effect {

    static {
        // This is the registration of the effect
        Skript.registerEffect(EffAddChannelModerator.class, "add [twitch] channel moderator %string%");
    }
    ...
    protected void execute(@NotNull Event event) {
        try {
            if (event instanceof BridgeEventChat) {
                ...
                client.getHelix().addChannelModerator(null, broadcasterID,userId).execute();
            }
        ...
        } catch (NullPointerException | HystrixRuntimeException e) {
            console.error("Error:[AddModeratorEffect] This error is triggered by one of the following reasons:\n" +
                "1.) User is not set.\n" +
                "2.) User is already a moderator.\n" +
                "3.) You don't have the broadcaster permission to add a moderator.");
        }
    ...
    public @NotNull String toString(Event event, boolean debug) {
        return "add [twitch] channel moderator " + exprUserName.toString(event, debug);
    }
