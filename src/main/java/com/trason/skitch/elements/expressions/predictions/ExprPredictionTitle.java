...
public class ExprPredictionTitle extends SimplePropertyExpression<Prediction.PredictionBuilder, String> {

    static {
        register(
            ExprPredictionTitle.class,
            String.class,
            "prediction title", // change here to change the expression itself (aka 'set prediction _title_ to ...')
            "prediction"
        );
    }
    ...
    public void change(Event e, @Nullable Object[] delta, Changer.ChangeMode mode) {
        if (delta == null || delta.length == 0 || delta.length > 45 || !(delta[0] instanceof String)) {
            Skript.warning("The input for title is wrong!");
            return;
        }

        final String title = (String) delta[0];
